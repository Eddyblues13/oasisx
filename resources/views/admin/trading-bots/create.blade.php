...
@section('title', 'Add Trading Bot - ' . config('app.name'))
@section('page-title', 'Add Trading Bot')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Add Trading Bot</h1>
        <p class="text-gray-500 dark:text-gray-400 text-sm">Create a new automated trading bot.</p>
    </div>

    <form action="{{ route('admin.trading-bots.store') }}" method="POST" enctype="multipart/form-data"
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:ring-black dark:focus:ring-white focus:border-black dark:focus:border-white text-sm">{{ old('description') }}</textarea>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="runtime_hours" id="runtime_hours" value="{{ old('runtime_hours', 1) }}"
                    min="1" required
                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:ring-black dark:focus:ring-white focus:border-black dark:focus:border-white text-sm">
            ...
            <div>
                <label for="hourly_roi" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Hourly
                    ROI (%)</label>
                <input type="number" name="hourly_roi" id="hourly_roi" value="{{ old('hourly_roi') }}" step="0.01"
                    min="0.01" required
                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:ring-black dark:focus:ring-white focus:border-black dark:focus:border-white text-sm">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label for="min_amount" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Min
                    Stake ($)</label>
                <input type="number" name="min_amount" id="min_amount" value="{{ old('min_amount') }}" step="0.01"
                    min="0.01" required
                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:ring-black dark:focus:ring-white focus:border-black dark:focus:border-white text-sm">
            ...
            <div>
                <label for="max_amount" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Max
                    Stake ($)</label>
                <input type="number" name="max_amount" id="max_amount" value="{{ old('max_amount') }}" step="0.01"
                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-white focus:ring-black dark:focus:ring-white focus:border-black dark:focus:border-white text-sm">
        ...
        <div>
            ...
            <input type="number" name="max_concurrent" id="max_concurrent" value="{{ old('max_concurrent', 2) }}"
        ...
        <div class="flex items-center gap-2">
            <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', true) ? 'checked' : ''
                ...
                class="rounded border-gray-300 dark:border-gray-600 text-black dark:text-white focus:ring-black dark:focus:ring-white">
