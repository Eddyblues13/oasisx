...
@section('title', 'Add Investment Plan - ' . config('app.name'))
@section('page-title', 'Add Investment Plan')
...
<div class="max-w-2xl">
    ...
    <div
        ...
        <div class="p-6 border-b border-gray-100 dark:border-gray-700">
            <h2 class="text-lg font-bold text-gray-900 dark:text-white">Add Investment Plan</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Create a new investment plan for users.</p>
        </div>
        <div class="p-6">
            <form action="{{ route('admin.investment-plans.store') }}" method="POST" class="space-y-5">
                @csrf

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Plan Name</label>
                        <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. Basic"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">ROI Percentage
                            (%)</label>
                        <input type="number" step="0.01" name="roi_percentage" value="{{ old('roi_percentage') }}"
                            required placeholder="3.00"
                            class="w-full px-3 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 dark:text-white focus:ring-black focus:border-black dark:focus:ring-white dark:focus:border-white">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" placeholder="Brief description of this plan"
