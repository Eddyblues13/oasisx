...
@section('title', 'Add Payment Method - ' . config('app.name'))
...
<div class="max-w-2xl">
    ...
    <div
        ...
        <div class="p-6 border-b border-gray-100 dark:border-gray-700">
            <h2 class="text-lg font-bold text-gray-900 dark:text-white">Add Payment Method</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Create a new deposit method for users.</p>
        </div>
        <div class="p-6">
            <form action="{{ route('admin.payment-methods.store') }}" method="POST" class="space-y-5"
                x-data="{ type: '{{ old('type', 'crypto') }}' }">
                @csrf

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Name</label>
                        <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. Bitcoin"
                    ...
                    <div>
                        ...
                        <input type="text" name="code" value="{{ old('code') }}" required placeholder="e.g. BTC"
                            maxlength="20"
                ...
                <div>
                    ...
                    <textarea name="instructions" rows="3" placeholder="Payment instructions shown to users"
                        class="w-full px-3 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 dark:text-white focus:ring-black focus:border-black dark:focus:ring-white dark:focus:border-white">{{ old('instructions') }}</textarea>
